...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="1,4,8,16,32",
        help="Comma-separated worker counts to sweep for TurboJPEG decode. Default: 1,4,8,16,32.",
    )
...
def default_output_path():
    ...
    return Path(__file__).resolve().parent / "reports" / f"resnet50_blackhole_trace_1cq_turbojpeg_threaded_preprocessing_e2e_{timestamp}.csv"
...
def create_jpeg_decoder(turbojpeg_lib_path=None):
    ...
    def get_decoder():
        if not hasattr(thread_local, "jpeg"):
            thread_local.jpeg = TurboJPEG(turbojpeg_lib_path)
        return thread_local.jpeg

    def decode_rgb(image_bytes):
        return get_decoder().decode(image_bytes, pixel_format=TJPF_RGB)
...
def decode_image_batch(decode_rgb, image_byte_batch, executor):
    if executor is None:
        return [decode_rgb(image_bytes) for image_bytes in image_byte_batch]
    return list(executor.map(decode_rgb, image_byte_batch))
...
def preprocess_image_batch(decode_rgb, image_byte_batch, executor):
    batch = resize_crop_decoded_images(decode_image_batch(decode_rgb, image_byte_batch, executor))
...
def measure_preprocessing(
    decode_rgb, image_bytes_list, batch_size, warmup_iterations, measurement_iterations, executor
):
    ...
    for iteration_index in range(warmup_iterations, warmup_iterations + measurement_iterations):
        ...
        last_inputs = preprocess_image_batch(decode_rgb, image_byte_batch, executor)
...
def main():
    ...
    try:
        ...
        for worker_count in worker_counts:
            executor_context = (
                ThreadPoolExecutor(max_workers=worker_count) if worker_count > 1 else nullcontext(None)
            )
            with executor_context as executor:
                logger.info(f"Benchmarking with preprocess_workers={worker_count}")
